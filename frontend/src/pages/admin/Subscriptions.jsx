...
export default function Subscriptions() {
  ...
  const packageOptions = useMemo(() => {
    return packages.map((p) => ({
      value: p.id,
      label: `${p.title} – ${Number(p.price).toLocaleString("vi-VN")}₫ (${p.duration_days} ngày)`,
    }));
  }, [packages]);
                ...
                : subs.map((s) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <td className="px-6 py-4">{daysLeftBadge(daysLeft)}</td>
            ...
            <div className="w-full max-w-lg rounded-3xl border border-slate-200 bg-white p-6 text-slate-900 shadow-2xl">
              <div className="mb-6 flex items-center justify-between border-b border-slate-200 pb-4">
                <div>
                  ...
                  <p className="mt-1 text-sm text-slate-500">
                    Chọn hội viên và gói tập để tạo đăng ký mới
                  </p>
                </div>
