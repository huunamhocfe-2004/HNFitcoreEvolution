...
const ProtectedRoute = ({ children, roles }) => {
  ...
  if (loading)
    return (
      <div className="h-screen bg-[#0a0a0a] flex flex-col items-center justify-center gap-4">
        ...
        <div className="text-yellow-500 font-bold tracking-widest text-xs uppercase animate-pulse">
          Đang tải Fitcore...
        </div>
      </div>
    );
  ...
  if (roles && !roles.includes(user.role)) {
    return (
      <Navigate to={user.role === "member" ? "/member" : "/admin"} replace />
    );
  }
  return children;
};
    ...
    <Routes>
      <Route
        ...
        element={
          user ? (
            <Navigate
              to={user.role === "member" ? "/member" : "/admin"}
              replace
            />
          ) : (
            <LoginPage />
