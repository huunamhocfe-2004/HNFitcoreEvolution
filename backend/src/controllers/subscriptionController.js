...
const create = async (req, res) => {
  ...
  if (!member_id || !package_id)
    return res
      .status(400)
      .json({ message: "Thiếu thông tin hội viên hoặc gói tập" });
  ...
  try {
    ...
    if (req.user.role === "member") {
      ...
      if (!mem.length || mem[0].id !== parseInt(member_id)) {
        return res
          .status(403)
          .json({ message: "Bạn không có quyền đăng ký cho người khác" });
      }
    ...
    const endD = new Date(
      new Date(startD).getTime() + pkg.duration_days * 86400000,
    )
      .toISOString()
      .slice(0, 10);
    ...
    if (promo_code) {
      ...
      if (Number(promo.discount_pct) > 0) {
        finalPrice = finalPrice * (1 - Number(promo.discount_pct) / 100);
      } else if (Number(promo.discount_amt) > 0) {
        finalPrice = finalPrice - Number(promo.discount_amt);
      }
    ...
    if (isPaid) {
      await pool.query('UPDATE members SET status="active" WHERE id=?', [
        member_id,
      ]);
    }
...
const markPaid = async (req, res) => {
  try {
    ...
    if (sub.is_paid)
      return res
        .status(400)
        .json({ message: "Gói tập này đã được thanh toán trước đó" });
...
const acceptRenewal = async (req, res) => {
  try {
    ...
    if (!rows.length)
      return res
        .status(404)
        .json({ message: "Không tìm thấy đề xuất gia hạn" });
    const sub = rows[0];
    if (sub.user_id !== req.user.id)
      return res
        .status(403)
        .json({ message: "Bạn không có quyền xử lý đề xuất này" });
    if (sub.is_paid)
      return res.status(400).json({ message: "Gói tập này đã được kích hoạt" });
    if (!["admin", "staff"].includes(sub.created_by_role)) {
      return res.status(400).json({
        message: "Yêu cầu này đã được gửi và đang chờ quản trị viên xác nhận",
      });
    }

    await pool.query("UPDATE subscriptions SET created_by = ? WHERE id = ?", [
      req.user.id,
      req.params.id,
    ]);
    res.json({
      message:
        "Đã đồng ý gia hạn. Vui lòng chờ quản trị viên xác nhận thanh toán.",
    });
  } catch (err) {
    console.error(err);
    res.status(500).json({ message: "Server error" });
...
const cancelRenewal = async (req, res) => {
  try {
    ...
    if (sub.is_paid)
      return res
        .status(400)
        .json({ message: "Không thể hủy gói đã kích hoạt" });
    if (!["admin", "staff"].includes(sub.created_by_role)) {
      return res.status(400).json({
        message: "Không thể hủy yêu cầu đang chờ quản trị viên xác nhận",
      });
    }
  ...
  try {
    ...
    if (package_id) {
      const [p] = await pool.query(
        "SELECT price FROM packages WHERE id = ? AND is_active = 1",
        [package_id],
      );
      ...
      if (Number(promo.discount_pct) > 0) {
        discount = (price * Number(promo.discount_pct)) / 100;
      } else {
        discount = Number(promo.discount_amt || 0);
      }
...
const createPromo = async (req, res) => {
  ...
  try {
    ...
    res
      .status(201)
      .json({ id: result.insertId, message: "Tạo mã khuyến mãi thành công" });
  } catch (err) {
    res.status(500).json({ message: "Server error" });
  }
};
...
const removePromo = async (req, res) => {
    try {
        ...
        if (err.code === 'ER_ROW_IS_REFERENCED_2') {
            return res.status(400).json({
                message: 'Mã giảm giá đã được sử dụng trong đăng ký, không thể xóa hẳn. Bạn nên ẩn mã này thay vì xóa.',
            });
        }
